...
export class TaskTypesDialogComponent implements AfterViewInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<TaskTypesDialogComponent>,
    @Inject(MAT_DIALOG_DATA) item: TaskType,
  ...
  save() {
    ...
    this.misc.SaveTaskType(newItem).subscribe({
      next: (item) => {
  ...
  delete() {
    this.misc.DeleteTaskType(this.item.ID).subscribe({
      next: () => {
